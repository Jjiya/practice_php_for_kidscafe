<?php class KakaoLogin
{
    ...
    /** * 인가코드를 이용해 토큰 요청 * * @param string $code * @return array */
    public
    ...
    { // 토큰 주소
        ...
        // 데이터
        $postData = array("grant_type" => "authorization_code", "client_id" => $this->cliend_id, "redirect_uri" => $this->callback_url, "code" => $code, "client_secret" => "");
        // API 호출
    ...
    /** * Access Token 을 이용해 사용자 정보 가져오기 * @param string $accessToken * @return array */
    public function getUserInfo($accessToken)
    {
        ...
        $arrResult = $this->call($headers, $url);
        $returnData = json_decode($arrResult[0], true);
        $retVal["code"] = $arrResult[3];
        $retVal["data"] = $returnData;
        return $retVal;
